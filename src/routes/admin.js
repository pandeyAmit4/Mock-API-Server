...
router.post("/validate-route", async (req, res) => {
	...
	try {
		...
		const duplicateRoute = routesConfig.find((r) => r.path === route.path && r.method.toUpperCase() === route.method.toUpperCase())
...
router.get("/routes", (req, res) => {
	...
	res.json({
		routes: routesConfig,
		hasSampleRoutes: routesConfig.some(route => 
			sampleRoutes.some(sample => 
				sample.path === route.path && 
				sample.method === route.method
			)
		)
	})
})
...
router.get("/storage/:resource", (req, res) => {
	try {
		...
		// Convert kebab-case to camelCase for path consistency
		const normalizedResource = resource.replace(/-([a-z])/g, (g) => g[1].toUpperCase())
		const path = `/api/${normalizedResource}`

		console.log("Fetching storage for path:", path)
		const data = StorageManager.getAll(path)
		// Keep original kebab-case for response
		const collectionKey = `${resource}s`

		if (!data || !data[collectionKey]) {
			return res.json({ [collectionKey]: [] })
		}
...
router.post("/reset/:resource", (req, res) => {
	try {
		...
		// Use original kebab-case for collection name
		const collectionKey = `${resource}s`
		let initialData = []
		if (route.response) {
			const template = route.response[collectionKey]?.[0]
			if (template) {
				initialData = Array(5)
					.fill(null)
					.map(() => ({
		...
		StorageManager.reset(path, { [collectionKey]: initialData })
